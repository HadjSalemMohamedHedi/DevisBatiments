<?php
	...
	if(isset($_GET['action']) && $_GET['action']=='delete') {
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			...
			redirect(ROOT_URL.'manage-colonnes.php');
		}
		
		if($db->update('colonnes',array('deleted'=>1),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La colonne a été supprimé avec succès');
	...
	/*delete trash*/
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Id invalide');
			redirect(ROOT_URL.'manage-colonnes.php?trash=true');
		}
		
		if($db->update('colonnes',array('deleted'=>2),$_GET['id'],true)) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La colonne a été supprimé avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
	...
	/*restore trash*/
		...
		if($db->update('colonnes',array('deleted'=>0),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La colonne a été restauré avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-colonnes.php?trash=true');
...
<html class=" ">
	...
	<body class=" ">
				...
				<section class="wrapper" style='margin-top:60px;display:inline-block;width:100%;padding:15px 0 0 15px;'>
					<div class='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
						<div class="page-title">
							<div class="pull-left">
								<h1 class="title">Gestion des types des colonnes</h1>
							</div>
						</div>
					</div>
								...
								<nav class="navbar navbar-default">
									<div class="container-fluid"> 
										...
										<div class="collapse navbar-collapse pull-right" id="bs-example-navbar-collapse-3">
											<button type="button" class="btn btn-default navbar-btn btn-icon" data-toggle="modal" onclick="Add();"> <i class="fa fa-plus-square"></i> &nbsp; <span>Ajouter un type</span></button>
										</div>
					...
					<div class="col-lg-12">
						<section class="box ">
							<header class="panel_header">
								<h2 class="title">Liste des colonnes</h2>
										...
										<table  class="table table-datatable table-custom" id="drillDownDataTable">
											<thead>
												<tr>
													...
													<th>Titre de La colonne (FR)</th>
													<th>Titre de La colonne (EN)</th>
		...
		<div class="modal fade" id="ultraModal-add">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-add" class="" action="./module_ajax/colonnes/validate.php" method="post">
		...
		<div class="modal fade" id="ultraModal-edit">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-edit" class="" action="./module_ajax/colonnes/validate.php" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 class="modal-title">Modifier La colonne</h4>
						</div>
		...
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				var oTable03 = $('#drillDownDataTable').dataTable({
					...
					"aaSorting": [[ 1, "asc" ]],
					"bProcessing": true,
					"sAjaxSource": "table/colonnes-types.php<?php echo $ajax_filter ?>",
					"aoColumns": [
					{ "mDataProp": "id" },
					{ "mDataProp": "titre_fr" },
					{ "mDataProp": "titre_en" },
			...
			<script type="text/javascript">
				...
				function delItem(id){
					...
					var a = confirm("Voulez-vous vraiment placer cette colone dans la Corbeille?");
					if(a){
				...
				function delTrash(id){
					...
					var a = confirm("Voulez-vous vraiment supprimer définitivement cette colone?");
					if(a){
				...
				function Add()
				{
					...
					jQuery.ajax({
						url: "module_ajax/colonnes/add.php",
				...
				function Edit(id)
				{
					...
					jQuery.ajax({
						url: "module_ajax/colonnes/edit.php?id="+id,
						success: function(response)
						{
