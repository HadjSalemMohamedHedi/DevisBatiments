...
if(isset($_GET['action']) && $_GET['action']=='delete') {
	...
	if($db->update('commandes',array('deleted'=>1),$_GET['id'])) {
...
/*delete trash*/
	...
	if($db->update('commandes',array('deleted'=>2),$_GET['id'],true)) {
		$_SESSION['notification'] = array('type'=>'succes','msg'=>'La commande a été supprimé avec succès');
	} else {
		$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
	}
...
/*restore trash*/
	...
	if($db->update('commandes',array('deleted'=>0),$_GET['id'])) {
...
<script type="text/javascript">
	...
	var oTable03 = $('#drillDownDataTable').dataTable({
		...
		"aaSorting": [[ 1, "asc" ]],
		"bProcessing": true,
		"sAjaxSource": "table/demandes-traite.php<?php echo $ajax_filter ?>",
		"aoColumns": [
		{ "mDataProp": "client" },
		{ "mDataProp": "Ref" },
		{ "mDataProp": "tableau" },
