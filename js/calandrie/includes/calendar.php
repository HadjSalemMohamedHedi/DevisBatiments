<?php
	...
	**************************************************************************/
	...
	class calendar
	{
		...
		*/
		public function json_transform($js = true)
		{
			...
			while($this->row = mysqli_fetch_array($this->result, MYSQL_ASSOC))
			{
				 ...
				 $event_id_responsible= $this->row['id_responsible'];
				 $event_title = ($this->row['title']!='')?$this->row['title']:$this->row['postcode']; 
				 ...
				 $event_statut = $this->row['statut'];
				 $event_client ='- <i class="fa  fa-user "></i> '.$this->row['client'].'<br>- <i class="fa  fa-phone "></i> '.$this->row['phone'].'<br>- <i class="fa  fa-envelope-o "></i> '.$this->row['email'].'<br>- <i class="fa  fa-map-marker"></i> '.$this->row['address']; 
				 $event_color = $this->row['color'];
				 $event_color2 = $this->row['color'];
			     $event_comment = ($this->row['comment']!='')?$this->row['comment']:'Pas de commantaire';
				 ...
				 if($js == true) 
				 {
					 ...
					 if($event_url == 'false' && $event_allDay == 'false')
					 {
						...
						$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color2' => $event_color2,'color' => $event_color, 'statut' => $event_statut,'client' => $event_client,'comment' => $event_comment);
						...
						array_push($this->json_array, $build_json);
						 ...
						 // Stores Each Database Record To An Array (Without URL)
						$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'color2' => $event_color2,'color' => $event_color, 'statut' => $event_statut ,'client' => $event_client,'comment' => $event_comment);
						...
						$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color' => $event_color,'color2' => $event_color2, 'url' => $event_url, 'statut' => $event_statut ,'client' => $event_client,'comment' => $event_comment);
						...
						array_push($this->json_array, $build_json);
						 ...
						 if($event_allDay == 'false') {
							...
							// Stores Each Database Record To An Array
							$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'color' => $event_color,'color2' => $event_color2, 'url' => $event_url,'comment' => $event_comment);
					...
					// When allDay = false the allDay options appears on the script, when its true it doesnot appear 
					 if($event_url == 'false' && $event_allDay == 'false')
					 {
						 ...
						 // Stores Each Database Record To An Array (Without URL)
						$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color' => $event_color,'color2' => $event_color2, 'statut' => $event_statut ,'client' => $event_client,'comment' => $event_comment);
						...
						array_push($this->json_array, $build_json);
						 ...
						 // Stores Each Database Record To An Array (Without URL)
						$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'color' => $event_color,'color2' => $event_color2, 'statut' => $event_statut ,'client' => $event_client,'comment' => $event_comment);
						...
						array_push($this->json_array, $build_json);
						 ...
						 if($event_allDay == 'false' && substr($event_url, -4, 1) == '.' || substr($event_url, -3, 1) == '.') { // domain top level checking
							...
							// Stores Each Database Record To An Array
							$build_json = array('id' => $event_id, 'title' => $event_title, 'description' => $event_description, 'start' => $event_start, 'end' => $event_end, 'color' => $event_color, 'color2' => $event_color2, 'url' => $event_url, 'statut' => $event_statut ,'client' => $event_client,'comment' => $event_comment);
		...
		* Returns true
		*/
		public function updates($id, $title, $description, $url,$id_commercial,$start_date,$start_time,$end_date,$end_time,$color,$allDay,$statut,$source)
		{			
			...
			$query = sprintf('UPDATE %s 
						...
						',
										mysqli_real_escape_string($this->connection, $this->table),
										mysqli_real_escape_string($this->connection, htmlentities($id_responsible)),
										...
										mysqli_real_escape_string($this->connection, htmlentities($statut)),
										mysqli_real_escape_string($this->connection, htmlentities($source)),
		...
		* Returns true
		*/
		public function addEvent($title, $description, $start_date, $start_time, $end_date, $end_time, $color, $allDay, $url, $id_commercial,$source,$statut,$extra=false)
		{			
			// Convert Date Time
			$start = $start_date.' '.$start_time.':00';
			$end = $end_date.' '.$end_time.':00';
			...
			{	
				...
				$query = sprintf('INSERT INTO %s 
							...
							',
											mysqli_real_escape_string($this->connection, $this->table),
											mysqli_real_escape_string($this->connection, htmlentities($id_client)),
											mysqli_real_escape_string($this->connection, htmlentities($id_avocat)),
											mysqli_real_escape_string($this->connection, htmlentities($title)),
											mysqli_real_escape_string($this->connection, htmlentities($description)),
											mysqli_real_escape_string($this->connection, htmlentities($start)),
											mysqli_real_escape_string($this->connection, htmlentities($end)),
