	...
	$nbr_gamme = get_nbr_gamme();
	function get_nbr_gamme(){
		$db = db_connect();
		$query="SELECT count(*) as nbr_gamme FROM categ where deleted = 0 ";
		$nbr_gamme = $db->get_row($query);
		return $nbr_gamme;
	}
	...
	$demande_traite = get_demande_traite();
	function get_demande_traite(){
		$db = db_connect();
		$query="SELECT count(*) as demande_traite FROM demandes where statut = 'validé' ";
		$demande_traite = $db->get_row($query);
		return $demande_traite;
	}

	$nb_commandes = get_commandes();
	function get_commandes(){
		$db = db_connect();
		$query="SELECT count(*) as nb_commandes FROM ordered_hx where statut = 1 and deleted =0";
		$nb_commandes = $db->get_row($query);
		return $nb_commandes;
	}


	$nbr_clients=get_nbr_clients();
	function get_nbr_clients(){
		$db = db_connect();
		$query="SELECT count(*) as nbr_clients FROM clients where id IN (SELECT id_client FROM demandes) and activate = 1 and deleted != 'deleted'";
		$nbr_clients = $db->get_row($query);
		return $nbr_clients;
	}

	$nbr_new_clients=get_nbr_new_clients();
	function get_nbr_new_clients(){
		$db = db_connect();
		$query="SELECT count(*) as nbr_new_clients FROM clients where activate = 0 and deleted != 'deleted'";
		$nbr_new_clients = $db->get_row($query);
		return $nbr_new_clients;
	}
								...
								<div class="overview-item item--c2">
									<div class="overview__inner">
										<div class="overview-box clearfix">
											...
											<div class="text">
												<span>Commandes en cours d'Expedition</span>
											</div>
										</div>
									</div>
								</div>
							...
							<a href="manage-products.php">
								<div class="overview-item item--c2">
									<div class="overview__inner">
										<div class="overview-box clearfix">
											<div class="icon">
												<i class="ti-shopping-cart-full c-pink"><span class="iconp">+</span></i>
