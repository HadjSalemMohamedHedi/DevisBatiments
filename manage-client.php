<?php
	...
	if(isset($_GET['action']) && $_GET['action']=='delete') {
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			...
			redirect(ROOT_URL.'manage-client.php');
		}
								...
								<nav class="navbar-default">
									<div class="container-fluid">
										...
										<button type="button" class="btn btn-default navbar-btn btn-icon btn-add-product" data-toggle="modal" onclick="Add();">
											 <i class="fa fa-plus-square"></i> &nbsp; <span>Ajouter un client</span>
											</button>
									</div>
									<!-- /.container-fluid --> 
								</nav>
		...
		<div class="modal fade" id="valid-client">
			<div class="modal-dialog" style="width:40%;">
				<div class="modal-content">
					...
					<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick="annulerClient();">&times;</button>
							<h4 class="modal-title">Valider le client </h4>
					</div>
 					<div class="modal-body" id="body_client"> Voulez-vous valider ce client ? </div>
					<div class="modal-footer"> 
						<button type="button" class="btn btn-white" data-dismiss="modal" onclick="annulerClient();">Annuler</button>
						<button type="submit" id="submit-edit" class="btn btn-info" onclick="validerClient();">Valider</button>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	/******************************************************/
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				var oTable03 = $('#drillDownDataTable').dataTable({
					...
					"aaSorting": [[ 1, "asc" ]],
					"bProcessing": true,
					"sAjaxSource": "table/clients.php<?php echo $ajax_filter ?>",
					"aoColumns": [
					{ "mDataProp": "id" },
					{ "mDataProp": "full_name" },
					{ "mDataProp": "email" },
		...
		function valider_client(id, etat)
		{
			...
			if(etat === 'desactiver'){
				$("#body_client").html('Voulez-vous désactiver ce compte ?');
			}else{
				$("#body_client").html('Voulez-vous valider ce client ?');
			}
		}
		...
		function validerClient(){
			...
			$.ajax({
				...
				success: function(data){
					if(data.match('success') != null){
						showSuccess('Le client a été validé');
