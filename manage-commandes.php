		...
		<? if($_SESSION['User']['role']=='superadmin'): ?><!-- rr-->
			...
			<section id="main-content" class=" ">
				<section class="wrapper" >
					<button type="button" class="btn btn-info" data-toggle="modal" data-target="#exampleModal" style="margin: 1%">
...
<form method="GET" id="FilterDate" name="f1">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Filtrer les commandes par date</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="margin-top: -17px;">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
       	 <div class="FilterDatediv">
							 	
							 	Date début : <input type="date" name="dateDeb" id="dateDeb" onchange="VerifDate()">
							 	Date Fin   : <input type="date" name="dateFin" id="dateFin" onchange="VerifDate()">
							 	<input type="hidden" name="Action" value="" id="StateVal">
...
<button type="submit" class="btn btn-info" id="Filtrer" disabled><i class="fa fa-filter"></i> Filtrer</button>      </div>
    </div>
  </div>
  	 	</form>
</div>
	...
	/******************************************************/
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				function VerifDate(){
						var dateDeb=document.f1.dateDeb.value;
						var dateFin=document.f1.dateFin.value;
						if(dateDeb>dateFin){
							console.log("dateFin <")
							document.getElementById("Filtrer").disabled = true;

						}else{
							console.log("dateFin >")
							document.getElementById("Filtrer").disabled = false;
						}
				}
				...
				var StateCmnd = "vide";
					...
					if((getUrlVars()["dateDeb"]!=="") && (getUrlVars()["dateDeb"]!=="")){
				dateDeb = getUrlVars()["dateDeb"];
				dateFin = getUrlVars()["dateFin"];
				StateCmnd = getUrlVars()["Action"];
				}
				...
				var Action = url.substring(url.lastIndexOf('?') + 1);
        			...
        			if(Action === "EnAtt"){
        				File="table/orders-en-attentes.php";
        				$('#StateVal').val("En préparation");
        			}else if(Action === "Expe"){
        				File="table/orders-en-expediee.php";
        				$('#StateVal').val("Expédié");
        			}else if(Action === "Liv"){
        				File="table/orders-livré.php";
        				$('#StateVal').val("Livrée");
        			}
					else{
						File="table/orders.php";
						$('#StateVal').val(decodeURIComponent(getUrlVars()["Action"]) );
						$('#dateDeb').val(dateDeb);
						$('#dateFin').val(dateFin);
					}
					...
					"sAjaxSource": "table/orders.php?dateDeb="+dateDeb+"&dateFin="+dateFin+"&Action="+StateCmnd,
