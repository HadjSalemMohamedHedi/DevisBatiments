...
if(isset($_GET['object'])&& $_GET['object']!=='' &&  isset($_GET['id'])&& is_numeric($_GET['id'])){
	...
	$object = $db->get_row("SELECT * FROM demandes WHERE  id='".$_GET['id']."'");
 	$client = $db->get_row("SELECT * FROM clients WHERE  id='".$object['id_client']."'");

 	$composants = $db->get_rows("SELECT * FROM composants WHERE id_demande='".$object['id']."'");

	if($object['statut'] == ''){$active = 'validé';}else{$active = '';}
	 
 
  $titre = 'La demande '.$object['titre'].' a été expédiée';
  $db->insert('notif',array('id_client' => $client['id'],'id_demande' => $object['id'], 'titre_fr'=> $titre, 'vue' => 0));

  $today = date("Y-m-d");

  	$table_pack = "<table width='80%' border='1' align='center' style='border-collapse: collapse;'><tr><th>Composant</th><th>Quantité</th></tr><tbody>";
  	foreach ($composants as $compo) {
  		$table_pack .= "<tr><td style='padding: 5px;text-align: center;'>".$compo['nom_composant']."</td><td style='padding: 5px;text-align: center;'>".$compo['quantite']."</td></tr>";
  	}
   $table_pack .= "</tbody></table>";

	if(!$db->update($_GET['object'],array('statut'=>$active, 'datetrai'=>$today),$_GET['id'])){
		echo 'error';
	}else{
		echo 'success';
		send_mail_to_client( $client['email'], $client['firstName'], $client['lastName'], $object['titre'], $object['ref'], $table_pack);
	}
...
function send_mail_to_client( $email, $nom, $prenom, $nom_pack, $ref_pack, $table_pack ){
...
<tr height="50"></tr>
 ...
 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#848282" data-bgcolor="BodyBg" data-module="02-Browser part" style="background-color: #d2d0d0;">
  ...
  <tr>
    ...
    <blockquote>
      <b>Votre demande a été expédiée</b>
    </blockquote>
 ...
 <tr>
    ...
    <blockquote>
      <b style="color: #e30613">Liste des composants</b>
    </blockquote>
