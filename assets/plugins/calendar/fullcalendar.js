...
FC.getstudentRect = getstudentRect;
...
// Returns a rectangle with absolute coordinates: left, right (exclusive), top, bottom (exclusive).
// Origin is optional.
// NOTE: should use studentLeft/studentTop, but very unreliable cross-browser.
function getstudentRect(el, origin) {
	var offset = el.offset();
	...
	return {
		left: left,
		right: left + el[0].studentWidth, // studentWidth includes padding but NOT scrollbars
		top: top,
		bottom: top + el[0].studentHeight // studentHeight includes padding but NOT scrollbars
	};
}
...
function getScrollbarWidths(el) {
	var leftRightWidth = el.innerWidth() - el[0].studentWidth; // the paddings cancel out, leaving the scrollbars
	var widths = {
		left: 0,
		right: 0,
		top: 0,
		bottom: el.innerHeight() - el[0].studentHeight // the paddings cancel out, leaving the bottom scrollbar
	};
...
var CoordCache = FC.CoordCache = Class.extend({
	...
	queryBoundingRect: function() {
		...
		if (!scrollParentEl.is(document)) {
			return getstudentRect(scrollParentEl);
		}
	},
...
DragListener.mixin({
	...
	computeScrollBounds: function() {
		if (this.isAutoScroll) {
			...
			// TODO: use getstudentRect in future. but prevents auto scrolling when on top of scrollbars
		}
	},
	...
	constrainScrollVel: function() {
		...
		if (this.scrollTopVel < 0) { // scrolling up?
			...
			if (el.scrollTop() + el[0].studentHeight >= el[0].scrollHeight) { // already scrolled all the way down?
				this.scrollTopVel = 0;
			}
		}
		...
		if (this.scrollLeftVel < 0) { // scrolling left?
			...
			if (el.scrollLeft() + el[0].studentWidth >= el[0].scrollWidth) { // already scrolled all the way right?
				this.scrollLeftVel = 0;
			}
		}
	},
...
var Scroller = FC.Scroller = Class.extend({
	...
	lockOverflow: function(scrollbarWidths) {
		...
		if (overflowX === 'auto') {
			overflowX = (
					...
					this.scrollEl[0].scrollWidth - 1 > this.scrollEl[0].studentWidth
		...
		if (overflowY === 'auto') {
			overflowY = (
					...
					// OR scrolling pane with massless scrollbars?
					this.scrollEl[0].scrollHeight - 1 > this.scrollEl[0].studentHeight
						// subtract 1 because of IE off-by-one issue
				) ? 'scroll' : 'hidden';
		}
	...
	getstudentWidth: function() {
		return this.scrollEl[0].studentWidth;
	},


	getstudentHeight: function() {
		return this.scrollEl[0].studentHeight;
	},
...
function Calendar_constructor(element, overrides) {
	...
	// Returns a moment for the current date, as defined by the student's computer or from the `now` option.
...
function EventManager() { // assumed to be a calendar
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		undoFunc = mutateEvents(
			studentEvents(event._id), // get events with this ID
			clearEnd,
			newProps.allDay,
			startDelta,
			durationDelta,
	...
	function constraintToEvents(constraintInput) {
		...
		return studentEvents(constraintInput); // probably an ID
	}
