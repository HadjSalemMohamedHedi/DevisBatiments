...
function openChrono(){
				var nb_chrono=1;
				var myWindow = window.open("./chrono.php", "", "height=670, width=420, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no");
				myWindow.moveTo((nb_chrono)*50, (nb_chrono)*50);
				//myWindow.resizeTo(416,460);
							}
...
jQuery(function($) {
         ...
         $('.page-topbar .sidebar_toggle').on('click', function() {
            ...
            if (menuarea.hasClass("collapseit") || menuarea.hasClass("chat_shift")) {
				...
				//img_logo
				 ...
				 $(".img_logo").attr("src","images/logo-hydrex-200.png");
            } else {
                ...
                ULTRA_SETTINGS.mainmenuCollapsed();
				 $(".img_logo").attr("src","images/icon-Hydrex.png");
        ...
        if ($.isFunction($.fn.fullCalendar)) {
			...
			$('#filter').fullCalendar({
				 dayClick: function(date, jsEvent, view) {
					...
					window.location.href='manage-prestation.php?date='+ date.format();
    ...
    ULTRA_SETTINGS.otherScripts = function() {
        ...
        if ($.isFunction($.fn.typeahead)) {
            ...
            var oscar_movies = new Bloodhound({
                datumTokenizer: Bloodhound.tokenizers.obj.whitespace('name'),
                queryTokenizer: Bloodhound.tokenizers.whitespace,
                remote: 'data/typeahead.php?q=%QUERY'
            });

            oscar_movies.initialize();

            $('#typeahead-5').typeahead(null, {
                    name: 'oscar-movies',
                    displayKey: 'value',
    ...
    ULTRA_SETTINGS.ultraToDoAddTaskWidget = function() {
        ...
        $(".wid-add-task textarea").on('keypress', function(e) {
			
			var note = $(this);
			
			var id_avocat = $('#id_avocat').val();
            ...
            if (e.keyCode == 13) {
				...
				$.ajax({
					type: "GET",
					dataType:"html",
					url: "./includes/add-note.php?note="+msg+"&id_avocat="+id_avocat+"&id_folder="+id_folder,
					success: function(data){
						
						id_note = parseInt(data);
				...
				var msg = '<li><a id="task-' + i + '" class="task btn btn-orange btn-xs btn-icon" data-id="'+id_note+'"><i class="fa fa-trash"></i>&nbsp;</a><a id="note-' + i + '" onclick="editNew();" class="edit-task btn btn-orange btn-xs btn-icon" data-id="'+id_note+'"><i class="fa fa-edit"></i>&nbsp;</a><label class="icheck-label form-label" for="task-' + i + '">' + msg + '</label></li>';
				
                note.parent().parent().find(".wid-all-tasks ul").append(msg);
                note.val("");
				showSuccess('Note ajouté avec succés');
                note.focus();
				...
				$('#note-' + i).click(function(){
                                                    ...
                                                    jQuery.ajax({
                                                        url: "edit-note.php?id_note="+id_note,
				...
				var task = $(this);
											...
											if(a){
												$.ajax({
													type: "GET",
													dataType:"html",
													url: "./includes/add-note.php?ac=delete&id_note="+id_note,
													success: function(data){
														showSuccess('Note supprimé avec succés');
														task.parent().fadeOut(200);
													}
												});
											}
										});
...
<!--partage-->
											function Share(id,object)
                                                {
                                                    jQuery('#share').modal('show', {backdrop: 'static'});

                                                    jQuery.ajax({
                                                        url: "includes/share.php?objet="+object+"&id="+id,
                                                        success: function(response)
                                                        {
                                                            jQuery('#share .modal-body').html(response);
...
$('.iswitch').on('change', function() {
					...
					$.ajax({
													type: "GET",
													dataType:"html",
													url: "./includes/change-statut.php?object="+object+"&id="+id,
													success: function(data){
