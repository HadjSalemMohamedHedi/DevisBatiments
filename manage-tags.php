<?php
	...
	if(isset($_GET['action']) && $_GET['action']=='delete') {
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			...
			redirect(ROOT_URL.'manage-tags.php');
		}
		
		if($db->update('tags',array('deleted'=>1),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'le tags a été supprimé avec succès');
	...
	/*delete trash*/
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Id invalide');
			redirect(ROOT_URL.'manage-tags.php?trash=true');
		}
		
		if($db->update('tags',array('deleted'=>2),$_GET['id'],true)) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'le tags a été supprimé avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
	...
	/*restore trash*/
		...
		if($db->update('tags',array('deleted'=>0),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'le tags a été restauré avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-tags.php?trash=true');
...
<html class=" ">
	...
	<body class=" ">
					...
					<div class="col-lg-12">
						<section class="box ">
							<header class="panel_header">
								<h2 class="title">Liste des tags</h2>
							</header>
		...
		<div class="modal fade" id="ultraModal-add">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-add" class="" action="./module_ajax/tags/validate.php" method="post">
		...
		<div class="modal fade" id="ultraModal-edit">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-edit" class="" action="./module_ajax/tags/validate.php" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 class="modal-title">Modifier le tags</h4>
						</div>
		...
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				var oTable03 = $('#drillDownDataTable').dataTable({
					...
					"aaSorting": [[ 2, "desc" ]],
					"bProcessing": true,
					"sAjaxSource": "table/tags.php<?php echo $ajax_filter ?>",
					"aoColumns": [
					{ "mDataProp": "id" },
					{ "mDataProp": "titre_fr" },
					{ "mDataProp": "nbr" },
			...
			<script type="text/javascript">
				...
				function delItem(id){
					...
					var a = confirm("Voulez-vous vraiment placer cette tags dans l'archive ?");
					if(a){
				...
				function delTrash(id){
					...
					var a = confirm("Voulez-vous vraiment supprimer définitivement cette tags?");
					if(a){
						...
						function(data){
							...
							if(data.match('success') != null){
								...
								window.setTimeout(function () {
									...
									$('#msg-add').hide();
									window.location.href = "manage-tags.php";
								}, 1000);
				...
				function Edit(id)
				{
					...
					jQuery.ajax({
						url: "module_ajax/tags/edit.php?id="+id,
						success: function(response)
						{
