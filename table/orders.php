<?php
	...
	$result = array("aaData"=>array());
		...
		if($Action==="En+préparation"){
			$Action="En préparation";

		}
	if(($_GET['dateDeb']!=="undefined")) {
		if(($_GET['Action']!=="undefined") && ($_GET['Action']!=="")){	
		$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0 and state = '".$Action."' and  date_create BETWEEN '".$_GET['dateDeb']."' and '".$_GET['dateFin']."'  ORDER BY id DESC");				
			}else{
				$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0   and date_create BETWEEN '".$_GET['dateDeb']."' and '".$_GET['dateFin']."'  ORDER BY id DESC");
			}
		...
		if($_GET['Action'] ==="undefined"){
			$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0  ORDER BY id DESC");
		}else{
		$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0 and state = '".$Action."'   ORDER BY id DESC");
	}
	}
	...
	}else if ($v['state'] == 'Livrée') {
		$statut =  '<h6 style="background-color: gainsboro; padding: 2%;width: 82%; margin-left: 8%;">Livraison confirmée</h6>';
	}
