<?
	...
	$rows = $db->get_rows("SELECT demandes.* FROM demandes where statut = '' ");
	foreach($rows as $x=>$v):
	...
	if(isset($_GET['trash']) && $_GET['trash']=='true'){
		...
		$action .= '<a type="button" class="btn btn-default navbar-btn  btn-icon btn-xs" data-toggle="modal" href="details-demande?id='.$v['id'].'" rel="tooltip" data-animate=" animated tada" data-toggle="tooltip" data-original-title="Détails de la demande" data-placement="top"> <i class="fa fa-info-circle"></i></a>';
	...
	if($v['statut']== 'validé'){ $statut .= ' checked="" '; }
	$statut .= 'class="iswitch iswitch-md iswitch-primary" onclick="valider_demande(\''.$v['id'].'\');">';
