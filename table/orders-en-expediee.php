<?php
	...
	$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0 and state = 'Expédié' and date_create BETWEEN '".$_GET['dateDeb']."' and '".$_GET['dateFin']."'  ORDER BY id DESC");
}

else{
	$rows = $db->get_rows("SELECT ordered_hx.* FROM ordered_hx WHERE deleted=0 and state = 'Expédié' ORDER BY id DESC");
