<?php
	...
	//$clients = $db->get_rows("SELECT * FROM clients WHERE id IN (SELECT id_client FROM audios) ORDER BY `id` DESC");

	$rows = $db->get_rows("SELECT clients.* FROM clients WHERE id IN (SELECT id_client FROM demandes) and deleted != 'deleted'");
	foreach($rows as $x=>$v):
	...
	if($v['status'] == 1){ $statut .= ' checked="" onclick="valider_client(\''.$v['id'].'\',\'desactiver\');"'; } 
	else{ $statut .= ' onclick="valider_client(\''.$v['id'].'\',\'activer\');" '; }
