<?php
	...
	if(isset($_GET['action']) && $_GET['action']=='delete') {
		...
		if($db->update('residence_type',array('deleted'=>1),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La couleur a été supprimé avec succès');
	...
	/*delete trash*/
		...
		if($db->update('residence_type',array('deleted'=>2),$_GET['id'],true)) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La couleur a été supprimé avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-residenceq.php?trash=true');
	}
	/*restore trash*/
		...
		if($db->update('residence_type',array('deleted'=>0),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La couleur a été restauré avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-residences.php?trash=true');
...
<html class=" ">
	...
	<body class=" ">
							...
							<div class="page-title">
								<nav class="navbar navbar-default">
									<div class="container-fluid"> 
										...
										<div class="collapse navbar-collapse pull-right" id="bs-example-navbar-collapse-3" style="    margin: 2%;">
											<a href="manage-residence-reservation-add.php" style="display: inline; " class="btn btn-default navbar-btn btn-icon"    > <i class="fa fa-plus-square"></i> &nbsp; <span>Ajouter une Réservation</span></a>
									<a href="manage-residence-reservation-calendar.php" style="margin-left:2%;display:inline;" class="btn btn-primary">Afficher la calendrier</a>
									</div>
					...
					<div class="col-lg-12">
						<section class="box ">
							<header class="panel_header">
								<h2 class="title pull-left">Liste des réservations</h2>
		...
		<div class="modal fade" id="ultraModal-add">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-add" class="" action="./module_ajax/residence/validate.php" method="post">
		...
		<div class="modal fade" id="ultraModal-edit">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-edit" class="" action="./module_ajax/residence/validate.php" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 class="modal-title">Modifier la résidence</h4>
						</div>
		...
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				var oTable03 = $('#drillDownDataTable').dataTable({
					...
					"aaSorting": [[ 1, "asc" ]],
					"bProcessing": true,
					"sAjaxSource": "table/reservation.php<?php echo $ajax_filter ?>",
					"aoColumns": [
					{ "mDataProp": "date_reservation" },
					{ "mDataProp": "ref" },
					{ "mDataProp": "client" },
	...
	/* 				{ "mDataProp": "payed" },
					...
					"fnInitComplete": function(oSettings, json) { 
						...
						$('.iswitch').on('change', function() {
							...
							$.ajax({
								type: "GET",
								dataType:"html",
								url: "./includes/set-check-reservation.php?object="+object+"&id="+id,
			...
			<script type="text/javascript">
				...
				function Add()
				{
					...
					jQuery.ajax({
						url: "module_ajax/residence/add.php",
						...
						function(data){
							...
							if(data.match('success') != null){
								...
								window.setTimeout(function () {
									...
									$('#msg-add').hide();
									window.location.href = "manage-residences.php";
								}, 1000);
				...
				function Edit_Prix(id)
				{
					window.location.href = "manage-residence-price.php?id="+id;
				}
				function Edit(id)
				{
					...
					jQuery.ajax({
						url: "module_ajax/residence/edit.php?id="+id,
						success: function(response)
						{
