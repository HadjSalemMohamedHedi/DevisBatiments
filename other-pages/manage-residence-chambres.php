<?php
	...
	if(isset($_GET['action']) && $_GET['action']=='delete') {
		...
		if($db->update('residence_chambres',array('deleted'=>1),$_GET['id'])) {
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La chambre a été supprimée avec succès');
		...
		redirect(ROOT_URL.'manage-residence-chambres.php?id_residence='.$_GET['id_residence']);
	}
	/*delete trash*/
		...
		if($db->update('residence_chambres',array('deleted'=>2),$_GET['id'],true)) {
		decrement_count_room($_GET['id_residence']);
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La chambre a été supprimée avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-residence-chambres.php?trash=true&id_residence='.$_GET['id_residence']);
	}
	/*restore trash*/
		...
		if($db->update('residence_chambres',array('deleted'=>0),$_GET['id'])) {
		increment_count_room($_GET['id_residence']);
			$_SESSION['notification'] = array('type'=>'succes','msg'=>'La chambre a été restauré avec succès');
			} else {
			$_SESSION['notification'] = array('type'=>'error','msg'=>'Il y a eu un problème S\'il vous plaît essayez de nouveau.');
		}
		redirect(ROOT_URL.'manage-residence-chambres.php?trash=true&id_residence='.$_GET['id_residence']);
...
<html class=" ">
	...
	<body class=" ">
				...
				<section class="wrapper" style='margin-top:60px;display:inline-block;width:100%;padding:15px 0 0 15px;'>
					<div class='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
						<div class="page-title">
							<div class="pull-left">
								<h1 class="title">Gestion des chambres de type <?php echo $residence_type['titre_fr'];?></h1>
							</div>
						</div>
					</div>
								...
								<nav class="navbar navbar-default">
									<div class="container-fluid"> 
										...
										<div class="collapse navbar-collapse pull-right" id="bs-example-navbar-collapse-3">
											<button type="button" class="btn btn-default navbar-btn btn-icon" data-toggle="modal" onclick="Add(<?php echo $_GET['id_residence'];?>);"> <i class="fa fa-plus-square"></i> &nbsp; <span>Ajouter une chambre de type <?php echo $residence_type['titre_fr'];?></span></button>
										</div>
					...
					<div class="col-lg-12">
						<section class="box ">
							<header class="panel_header">
								<h2 class="title pull-left">Liste des chambres : <?php echo $residence_type['titre_fr'];?></h2>
		...
		<div class="modal fade" id="ultraModal-add">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-add" class="" action="./module_ajax/residence-chambres/validate.php" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 class="modal-title">Ajouter une chambre de type <?php echo $residence_type['titre_fr'];?></h4>
						</div>
						<div id="msg-add" style="padding:15px; display:none"></div>
						<div class="modal-body" > loading... </div>
						<div class="modal-footer"> 
							<button type="button" class="btn btn-white" data-dismiss="modal">Fermer</button>
							<button type="submit" id="submit-add" class="btn btn-info">Ajouter</button>
						</div>
					</form>
				</div>
			</div>
		...
		<div class="modal fade" id="ultraModal-edit">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form-edit" class="" action="./module_ajax/residence-chambres/validate.php" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
						...
						<div class="modal-footer"> 
							<button type="button" class="btn btn-white" data-dismiss="modal">Fermer</button>
							<button type="submit" id="submit-edit" class="btn btn-info">Modifier</button>
						</div>
					</form>
				</div>
			</div>
		...
		/**************** DRILL DOWN DATATABLE ****************/
			/******************************************************/
				...
				var oTable03 = $('#drillDownDataTable').dataTable({
					...
					"aaSorting": [[ 1, "asc" ]],
					"bProcessing": true,
					"sAjaxSource": "table/residence-chambres.php<?php echo $ajax_filter ?>",
					"aoColumns": [
					{ "mDataProp": "numero" },
					...
					"fnInitComplete": function(oSettings, json) { 
						...
						$('.iswitch').on('change', function() {
							...
							$.ajax({
								type: "GET",
								dataType:"html",
								url: "./includes/change-statut-chambre.php?id_residence=<?php echo $_GET['id_residence'];?>&object="+object+"&id="+id,
			...
			<script type="text/javascript">
				...
				function delItem(id){
					...
					var a = confirm("Voulez-vous vraiment placer cette chambre dans la Corbeille?");
					if(a){
				...
				function delTrash(id){
					...
					var a = confirm("Voulez-vous vraiment supprimer définitivement cette chambre?");
					if(a){
				...
				function Add(id_residence)
				{
					...
					$('#msg-add').html('');
					jQuery.ajax({
						url: "module_ajax/residence-chambres/add.php?id_residence="+id_residence,
				...
				function Edit(id)
				{
					...
					$('#msg-edit').html('');
 					jQuery.ajax({
						url: "module_ajax/residence-chambres/edit.php?id="+id,
						success: function(response)
						{
