<?php
	...
	$Newpath = '../../../test/'.$id_product.'/'; // upload directory
	$Newpath_s = '../../../test/'.$id_product.'/s/'; // upload directory

	if (!file_exists($path)) {mkdir($path, 0777, true);mkdir($Newpath, 0777, true);}
	...
	if(isset($_FILES['image']))
	{
		...
		{					
			...
			$Newpath = $Newpath.strtolower($final_image);	
			$Newimg_s = $Newpath.strtolower($final_image);

			
			// try {
			//    	if (move_uploaded_file($tmp,$Newpath)){
			// 				$_SESSION['images-product'][]=$Newpath;
			...
			if(move_uploaded_file($tmp,$Newpath)) 
			{
				...
				if (!file_exists($Newpath_s)) {
					//mkdir($path_s, 0777, true);
					mkdir($Newpath_s, 0777, true);
				}
				...
				$image->save($Newpath_s.$prefix.$final_image);
				
				
				$data = getimagesize($Newpath_s.$prefix.$final_image);
				$width = $data[0];
				$height = $data[1];
				...
				$image->load($Newpath_s.$prefix.$final_image);
